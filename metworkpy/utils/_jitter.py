...
def _jitter(x: np.ndarray,
            ...
            jitter_seed: int,
            discrete_x: bool,
            discrete_y: bool) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    :type y: np.ndarray
    :param jitter: Standard deviation of noise to add. Can be a single float which is used as the standard deviation
        for the noise added to both arrays, or a tuple of floats where the first value is used for the noise applied to
        x, and the second for y.
    :type jitter: Union[float, tuple[float,float]]
    :param jitter_seed: Seed for random number generator used to generate the noise
