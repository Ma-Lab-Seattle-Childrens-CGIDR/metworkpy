...
Thank you for your interest in contributing to MetworkPy!
...
(This is documentation for maintaners, you don't have to worry about this just
to open a pull request).

Releases of MetworkPy are automatically generated by the CI when pushing to
github, specifically when there is a push to a branch named vX.X.X, where X is
some number. So the steps to create a release are:

1. Pull/merge all changes for the release onto the main branch
1. Update the version in the pyproject.toml file (this will automatically update
   the \_\_version\_\_ variable)
1. Create a new vX.X.X branch and switch to it
1. Set the upstream of the branch
1. Push the branch to Github, this will trigger the CI to first run the test
   suite, and if that passes to create a release and upload it to PyPI
