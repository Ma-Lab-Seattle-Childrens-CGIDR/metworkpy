...
class TestRun(unittest.TestCase):
    default_dict = {
        ...
        "gene_expression_file": BASE_PATH / "data" / "test_model_gene_expression_metchange.csv",
        "wildtype": "0:5",
        "sample_groups": "",
    ...
    def setUpClass(cls):
        ...
        cls.gene_expression = pd.read_csv(cls.data_path / "test_model_gene_expression_metchange.csv",
    ...
    def run_cli(self, **kwargs) -> pd.DataFrame:
        ...
        with mock.patch("argparse.ArgumentParser.parse_args",
            ...
            # Read the results in
            metchange_results = pd.read_csv(argparse.ArgumentParser.parse_args().output_file,
                                            index_col=0, header=0)
    ...
    def match_cli(self, **kwargs):
        ...
        with mock.patch("argparse.ArgumentParser.parse_args",
            ...
            self.assertTrue(os.path.exists(argparse.ArgumentParser.parse_args().output_file))
            # Read the results in
            metchange_results_cli = pd.read_csv(argparse.ArgumentParser.parse_args().output_file,
                                                index_col=0, header=0)
            ...
            for s in wt_samples:
                wt_gene_weights = metworkpy.utils.expr_to_metchange_gene_weights(
                    ...
                    subset=(None if not namespace_dict["subset"] else self.test_model.genes.list_attr("id")),
            ...
            if namespace_dict["sample_groups"]:
                ...
                sample_groups = [_script_utils._parse_samples(m)
                                 for m in group_pattern.findall(namespace_dict["sample_groups"])]
            else:
                sample_groups = [[i] for i in range(len(self.gene_expression.index)) if i not in wt_samples]
                sample_names = list(self.gene_expression.index[list(itertools.chain.from_iterable(sample_groups))])
            ...
            # Run Metchange
            wt_metchange = pd.DataFrame(0., index=range(len(wt_rxn_weights)),
            ...
            metchange_results_expected = pd.DataFrame(0.,
                                                      index=sample_names,
                                                      columns=self.test_model.metabolites.list_attr("id"))
        ...
        self.assertTrue(np.isclose(metchange_results_cli.fillna(0.).values,
                                   metchange_results_expected.fillna(0.).values).all())
    ...
    def test_metabolites_file(self):
        with open(BASE_PATH / "data" / "met_list.txt", "r") as f:
            met_file = f.read()
        metchange_res = self.run_cli(metabolites_file=BASE_PATH / "data" / "met_list.txt")
    ...
    def test_sample_groups(self):
        ...
        res = self.run_cli(sample_groups="(6,7)(8)", sample_group_names=sample_group_names)
