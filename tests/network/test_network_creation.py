...
class TestAdjMatDWFlux(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.tiny_adj_mat = _adj_mat_d_w_flux(cls.tiny_model,
                                             rxn_bounds=(rxn_min, rxn_max))
...
class TestAdjMatUdWFlux(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.tiny_adj_mat = _adj_mat_ud_w_flux(cls.tiny_model,
                                              rxn_bounds=(rxn_min, rxn_max))
...
class TestCreateAdjacencyMatrix(unittest.TestCase):
    ...
    def test_undirected_unweighted(self):
        adj_mat, _, _ = create_adjacency_matrix(model=self.test_model,
                                                directed=False,
                                                weighted=False,
                                                out_format="csr")
    ...
    def test_directed_weighted_flux(self):
        adj_mat, _, _ = create_adjacency_matrix(model=self.test_model,
                                                ...
                                                weight_by="flux",
                                                out_format="csr")
        adj_mat_known = _adj_mat_d_w_flux(model=self.test_model,
                                          rxn_bounds=self.test_model_rxn_bounds)
        self.assertTrue(np.isclose(adj_mat.toarray(), adj_mat_known.toarray()).all())
    ...
    def test_undirected_weighted_flux(self):
        adj_mat, _, _ = create_adjacency_matrix(model=self.test_model,
                                                ...
                                                weight_by="flux",
                                                out_format="csr")
        adj_mat_known = _adj_mat_ud_w_flux(model=self.test_model,
                                           rxn_bounds=self.test_model_rxn_bounds)
        self.assertTrue(np.isclose(adj_mat.toarray(), adj_mat_known.toarray()).all())
    ...
    def test_loopless(self):
        adj_mat_loopless, _, _ = create_adjacency_matrix(model=self.tiny_model,
                                                         directed=True,
                                                         weighted=True,
                                                         weight_by="flux",
                                                         out_format="csr",
        ...
        adj_mat, _, _ = create_adjacency_matrix(model=self.tiny_model,
                                                directed=True,
                                                weighted=True,
                                                weight_by="flux",
                                                out_format="csr",
