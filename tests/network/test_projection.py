...
class TestBipartiteProject(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        dg.add_edges_from([("A", "D"), ("B", "D"), ("B", "E"), ("D", "C"), ])
        cls.test_digraph = dg

        # Create weighted test graph
        wg = nx.DiGraph()
        wg.add_nodes_from(["A", "B", "C", "D", "E"])
        wg.add_edges_from([("A", "D", {"weight": 5}), ("B", "D", {"weight": 2}),
                           ("B", "E", {"weight": 2}), ("D", "C", {"weight": 3})])
    ...
    def test_unweighted_directed(self):
        directed_projection = bipartite_project(self.test_digraph,
        ...
        directed_projection2 = bipartite_project(self.test_digraph,
                                                 node_set=self.node_set)
        self.assertTrue(nx.utils.graphs_equal(directed_projection,
                                              directed_projection2))
        # Test conversion to undirected
        undirected_projection = bipartite_project(self.test_digraph,
                                                  node_set=self.node_set,
                                                  directed=False)
        ...
        self.assertTrue(
            nx.utils.graphs_equal(
                ...
                nx.complete_graph(["A", "B", "C"])))

    def test_weighted_directed(self):
        weighted_projection = bipartite_project(self.test_weighted_graph,
                                                node_set=self.node_set,
                                                directed=True,
                                                weight=min,
