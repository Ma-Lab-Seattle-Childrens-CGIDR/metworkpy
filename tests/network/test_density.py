...
class TestLabelDensity(unittest.TestCase):
    ...
    def test_node_density(self):
        node_density_calc1 = _node_density(self.test_graph,
        ...
        node_density_calc2 = _node_density(self.test_graph,
                                           labels=pd.Series(self.test_labels),
                                           node=6, radius=1)
        node_density_expected2 = 0.
        self.assertTrue(np.isclose(node_density_calc2, node_density_expected2))

    def test_label_density(self):
        label_density_calc = label_density(self.test_graph,
                                           labels=self.test_labels,
                                           radius=1)
        label_density_exp = pd.Series({
            0: 0.5, 1: (5 / 3), 2: (2 / 4), 3: (3 / 4),
            4: 0, 5: (5 / 4), 6: 0, 7: (5 / 2), 8: (2 / 2)
        }).sort_index()
...
class TestFindDenseClusters(unittest.TestCase):
    ...
    def test_find_dense_clusters(self):
        res_df = find_dense_clusters(network=self.test_graph,
                                     labels=self.test_labels,
                                     radius=0,
                                     quantile_cutoff=3 / 9)
        for i in [0, 5, 7]:
