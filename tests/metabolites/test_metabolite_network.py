...
class TestMetaboliteObjective(unittest.TestCase):
    ...
    def test_adding_metabolite_objective(self):
        ...
        with MetaboliteObjective(model=self.model,
                                 metabolite="F_c") as m:
            self.assertEqual(
...
class TestFindMetaboliteNetwork(unittest.TestCase):
    ...
    def test_find_metabolite_network_reactions(self):
        ...
        original_model = self.model.copy()
        ess_met_net = find_metabolite_network_reactions(model=self.model,
                                                        method="essential",
                                                        essential_proportion=0.05)
        self.assertTrue(model_eq(self.model, original_model))
        ess_f = ess_met_net["F_c"]
        self.assertEqual(ess_f.dtype, "bool")
        for rxn in ["R_A_e_ex", "R_B_e_ex", "R_C_e_ex", "R_G_e_ex",
                    "R_A_imp", "R_B_imp", "R_C_imp", "R_G_exp",
                    "r_A_B_D_E", "r_C_E_F", "r_D_G"]:
    ...
    def test_find_metabolite_network_reactions_pfba(self):
        original_model = self.model.copy()
        pfba_met_net = find_metabolite_network_reactions(model=self.model,
                                                         method="pfba",
                                                         pfba_proportion=1.)
        self.assertTrue(model_eq(self.model, original_model))
        ...
        for rxn in ["R_G_e_ex",
            ...
            self.assertAlmostEqual(pfba_f[rxn], 50)
        for rxn in ["R_F_e_ex", "R_H_e_ex", "R_F_exp", "r_C_H", "R_H_exp"]:
            self.assertAlmostEqual(pfba_f[rxn], 0.)

    def test_find_metabolite_network_genes_essential(self):
        # Test essential method
        ess_met_net = find_metabolite_network_genes(model=self.model,
                                                    method="essential",
                                                    essential_proportion=0.05)
        ess_f = ess_met_net["F_c"]
        for gene in ["g_A_imp", "g_B_imp", "g_C_imp", "g_G_exp",
                     "g_A_B_D_E", "g_C_E_F", "g_D_G", ]:
    ...
    def test_find_metabolite_network_genes_pfba(self):
        # Test pfba method
        pfba_met_net = find_metabolite_network_genes(model=self.model,
                                                     method="pfba",
                                                     pfba_proportion=1.)
        pfba_f = pfba_met_net["F_c"]
        for gene in ["g_A_imp", "g_B_imp", "g_C_imp", "g_G_exp", "g_A_B_D_E",
                     "g_C_E_F", "g_D_G", ]:
